...
- `LLM_CALL_STARTED/COMPLETED` 的 `model`：loop 没配 `model` 时取服务自己的模型名
  （此前是空串——宿主的逐次调用账 61% 的行因此记不上是哪个模型，design/124 U6）。
...
- 🔴 **思考模式下的一个空回合会让整个 run 降级**（真实事故 conv266：问答子 agent 4 轮后死在这里）。
  一条链上三个缺陷，各修各的：

  1. **`reasoning_content` 没有回传给供应商**。带过思考的 assistant 消息，DeepSeek 的思考模式要求
     下一次请求把它原样带回去，少了就是 400
     「The `reasoning_content` in the thinking mode must be passed back to the API」。
     那是**硬错**——重试同样的历史只会再错一遍，最后 `retry_exhausted` 整个 run 降级。
     现在只在「这次回复真的带了思考」时回传；没带的（OpenAI 正统、Anthropic 走自己的 thinking 块）
     一个字段都不加，行为不变。

  2. **「只有思考、没有正文也没有工具调用」被当成打嗝原样重试**。输入不变输出不会变，重试只是
     再烧一轮同样的预算。已有的截断判据依赖 `finish_reason` 与 `usage`，而**流式响应里这两样
     常常都取不到**，于是双双落空、掉进空回复重试那条路。新增第三条判据：有思考、无正文、
     无工具调用 → 按截断处理，补一条「你上一轮只产出了思考，直接给结论」的提示改变输入，不重试。

  3. **三样都没有的那一轮不再往历史里塞一条空 assistant 行**。它一个字的信息都不带，却会让
     下一次请求以一条空 assistant 消息结尾——在思考模式下那是非法的，别的供应商也只是白占一行。

### Changed

- **BREAKING（轻微）**：`AgentSpec.max_tokens` 默认从 `4000` 改为 `None` = **跟父 loop 一样**。
  它不是「答案能多长」——**思考内容也算在里面**，开了思考的模型被压在小预算里会把预算全花在思考上，
  正文一个字都没有。真实事故里子 agent 是 3000、父是 40000，而两者用的是同一个模型。
  显式传数字的调用方行为不变；`workflow` 的叶子没有父 loop 概念，回落 4000。
...
- **send 内保险丝按「本 send 的增长」触发，不再按整个上下文**。此前 `insend_distill_tokens` 拿上一轮真实
  `prompt_tokens`（= 系统提示词 + 技能 + 工具目录 + 历史 + 本 send）整体去比，一个 3 万 token 的开局
  配 30000 的阈值就是第 1 轮触发、之后每轮都烧（真实事故 conv-237：一个会话 44 次触发）。现在第一轮
  的真实 prompt_tokens 是基线，之后每轮减它；拿不到真实用量就估算本 send 新增的行。第 0 轮永不触发。
- **保险丝只动本 send 追加的行**。run 之前就在历史里的（上一个 send 留下的逐字行）归跨 send 投影管，
  两套机制不再抢同一段历史。
- **`insend_distill_keep_tools`**：登记的工具调用参数永不瘦身（结果照旧）。给「模型自己说的话」用——
  `send_message` / `post_update` 的参数就是发言，瘦成「⟨已移出上下文 N 字符…⟩」后模型会照着占位符的
  样子再发一遍（conv-237：九条占位符被当正文发进了聊天）。宿主按工具名登记，默认空。
...
- **send 内保险丝也回收工具调用参数**，不再只压结果。工具在上下文里占两块：结果在 tool 行、
  **参数在 assistant 行的 `tool_calls` 里**。参数此前从没被保险丝碰过，而实测它占全库上下文的
  **29%**——`write_file` 一个人 12.6 MB、单次最大 5.4 万字符：文件已经落盘了，正文却还在 prompt
  里一轮轮重发。跨 send 的投影早就把两块一起收掉（`wrote <文件名>`），缺的只是 send 内这一层。
  两块现在按位置统一排队、一起算进 `insend_distill_batch`，最早的先回收。
  瘦身规则通用（不套投影那套逐工具散文规则，因为这里改的是供应商要按 JSON 解析的
  `function.arguments`）：短字段原样留下——路径、模式、行号这些才看得出这次调用干了什么——
  只有超过 200 字符的长字符串换成一句带 `recall_send` 坐标的说明，**结果仍是合法 JSON**。
  工具早已执行完，改参数不产生任何副作用，只影响模型回看时读到什么；没省下字符就不动
  （改了反而打断供应商的前缀缓存）。
