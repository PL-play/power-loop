...
async def test_resume_twice_replays_across_both_prior_attempts() -> None:
    ...
    try:
        ...
        await journal_mod.seed(lp.store, sid, run_id, spec.name, spec=spec.to_dict())
        ...
        done = {s["node_id"] for s in (await journal_mod.read(lp.store, sid, run_id))["steps"]
        ...
        assert (await journal_mod.read(lp.store, sid, run_id))["attempts"] == 3
    finally:
        await lp.aclose()
...
async def test_resume_precharges_spec_defined_budget() -> None:
    ...
    try:
        ...
        await journal_mod.finalize(lp.store, sid, run_id, r1)
        done = {s["node_id"] for s in (await journal_mod.read(lp.store, sid, run_id))["steps"]
                if s["status"] == "completed"}
        assert "a" in done
